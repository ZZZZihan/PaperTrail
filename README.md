...
**2026-09-06 最新实测：** 已完成真实浏览器问答与 Reflexion 研读卡更新，新增 10 次 Sol 调用；此前连通性测试新增 3 次，当前同 scope 累计 **407/407**。正常问答与新卡通过独立原文核对；不足题未编造数值，但返回旁支背景和部分回答，未达预设不足行为。应用/数据库重启后全部数据一致，详见 [真实端到端验证](docs/verification-live-e2e-2026-09-06.md)。下方 394/394 为 2026-09-05 历史交付快照，后续新增调用按当前任务授权安排。
...
本地入口：[PaperTrail](http://127.0.0.1:8000/)，也可直接打开 [ReAct 阅读页](http://127.0.0.1:8000/#paper=acffff0b-0513-4b00-98d3-40567542eda9&page=0)。优先阅读已保存的简介与问答，查看和刷新不调用模型。旧回答不会被补写覆盖成绩；旧简介可以主动“补全为研读卡”，会调用模型，升级中或失败时仍可阅读原简介。当前任务 [COL-19](https://linear.app/colife/issue/COL-19) 为 In Review，PR 仍为 draft、未合并，产品与学习认可待用户试读。
...
提交前审查与扩展浏览器/API 端到端结果见 [审查验证记录](docs/verification-review-2026-09-05.md)。`make check-e2e` 可在独立 OFFLINE 环境重复验证完整 HTTP、持久化与重启流程；不使用现有论文库和真实模型额度。
