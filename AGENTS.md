...
- 2026-09-05 最新授权：用户明确“暂时批准你不限量去使用”，解除此前同账本 167 次上限导致的阻塞。本次 Goal 的必要开发测试可临时使用 `PAPERTRAIL_MODEL_MAX_CALLS=unlimited`；仍使用现有 Sol、`provider_quota` 与原 scope，逐次保留用量和未知费用，保留每任务调用次数、超时及唯一一次内容修订。此前 160/167 是历史授权，不能再作为本轮新增测试的阻塞。临时授权在本 Goal 工程验证完成或用户撤销时结束，届时关闭不限量并将数值上限设为实际累计次数；不扩展到无关项目或任务。
...
- 当前真实进展：QA v7 evidence-qa-v7-query-term-coverage 根据真实合并检索遗漏加入最多两条词覆盖补充（仍12段/20k），实现前冻结同条件报告，349项make check通过；v7全旧集独立8/10完整、17/18要点、11/11条件、72/73事实有据，旧失败保留。4篇20题留出已执行并独立评分：4/16严格完整、43/62点、18/29条件、135/135已发布事实、3/20超时；不据其调参。简介v9三篇中仅Reflexion发布，但独立自身引文支持仅5/8；ReAct/Toolformer未发布，本批无完整支持的新发布卡。工程完成仍pending，实际版本/分轮结果/用量统一见 docs/verification-quality-v02.md。
- 本轮开始前账本快照为同 scope 153/160；这不是实时余量。2026-09-05 用户随后明确批准原 react-03 最多 3 次与 ReAct v6 卡最多 4 次测试，当时原账本累计上限增为 167 次（后续临时不限量授权已取代该限制）；只修改上限，scope、模式和模型保持，真实运行先核对当前余量，不能换 scope、重置或换模式绕过；未知费用保持未知。4 篇 20 题候选留出集已冻结来源与规则，AI 准备和人工核对分别标记，完整模型评测未执行不记完成。工程工作继续，产品认可、人工样例核对与学习掌握均待本人实际反馈，不把等待反馈当作停止工程工作的条件。
- 前置阶段（以下保留历史）：
