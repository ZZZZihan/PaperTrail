...
- 当前修复代码 `248a50d04d7ffa00ad75eab11de366d786510f72` 的 make check / 310 项后端测试通过，真实复验已获新增最多 7 次调用授权、正在执行，准确性修复尚未判为完成：`8fb80483c9103101ff8c0e8c4b5923cb95add10d` / `evidence-qa-v4-coverage` 的新 react-03 真实运行仍漏模型冻结、人工编排轨迹条件，已正确返回 partial_answer。冻结条件是检索缺口，人工轨迹是生成覆盖缺口，继续按证据修复；不得以部分回答标记正确替代完整回答目标。新 ReAct `paper-introduction-v5` 真实诊断末次检查因总期限超时失败，旧成功卡继续可读，新卡待再验证。旧 v3/v4 成绩与失败保持原样。
