...
当前使用同账本临时不限量继续必要测试。QA v7 的旧 15 题独立回归为 8/10 完整有据交付、17/18 原要点、11/11 必要条件、72/73 原子事实有据；仍有生成遗漏、完整性误判和一处背景范围错误。新 4 篇 20 题留出独立评分为 4/16 严格完整、43/62 要点、18/29 条件，原批次含 3/20 超时；已发布事实有据不代表完整回答，不将留出用于调参。简介 v9 尚无完整独立支持的新发布卡。版本、分轮结果及未完成项统一见 [v0.2 验证记录](verification-quality-v02.md)；人工产品与学习验收仍待本人反馈。
